...
struct HashTable{
	...
	//add an if condition to complete the code here
        if (empty){      //If the slot is empty , then update the slot by new username giving the password
            password[hash] = user_password;
        }
        else{
            cout<<"The following slot is already occupied !\nTry another user name."<<endl;  //If the slot is already occupied , then give the error message 
        }
